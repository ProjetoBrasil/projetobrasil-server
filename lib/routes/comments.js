...
exports.update = function (req, res){
  var comment = politicianModel.updatePolitician(req);

  return ddb.updateItem('comments', req.params.id, null, comment, function (err, comments, cap) {
      if (!err) {
        console.log("updated");
      } else {
        console.log(err);
      }
      return res.send(202, comments);
  });
};
