	...
	mysql.query('SELECT * from duplasUFC where duplasUFC.id not in (select id_dupla from notasUFC where notasUFC.id_user = ?) and id_tema = ? ',
		...
		if (err){
			...
			mysql.query('SELECT * from propostasUFC where id  in (?,?)',
				...
				res.send([proposals[i], proposals[(i+1)%2]]);
			});
		}
	});
};
...
exports.saveUFCVote = function(req, res){
	...
	mysql.query('INSERT INTO notasUFC (id_user, id_proposta_ganhadora, id_dupla, created_at) VALUE (?,?,(SELECT id FROM duplasUFC WHERE id_proposta1 IN(?,?) AND id_proposta2 IN(?,?)), NOW())',
		[req._passport.session.user, req.body.propostas[0], req.body.propostas[1], req.body.propostas[0], req.body.propostas[0], req.body.propostas[1]],
...
exports.profileHashRankUFC = function(req, res){
	mysql.query('SELECT COUNT(id_user) as quantidade, id_politicians, id_tema from (select notasUFC.* from notasUFC inner join userHashUFC on notasUFC.id_user = userHashUFC.id where hash = ?) as notas right join propostasUFC on id_proposta_ganhadora = propostasUFC.id group by id_tema, id_politicians',
		[req.params.hash],function(err, rows, fields) {
		if (err) console.log(err);
		var resposta = {};
...
exports.importPropostas = function(req, res){
	res.send(202);
	var temasUFC = {
	...
	ddb.scan('proposals', {}, function (err, proposals) {
		if (!err) {
			proposals.items.forEach(function(proposal, index){
				var subtema = proposal.pares || proposal.subtema || proposal.tema || '';
				if(proposal.politicians_id == "827c9cc0-0d10-11e4-a4de-3d18690f2356" || proposal.politicians_id == "b6bc0250-0d10-11e4-b416-b9cab1b63b1e"){
					mysql.query('INSERT INTO propostasUFC (id, id_politicians, id_tema, titulo, subtema) VALUE (?, ?, ?, ?, ?)',
					[proposal.id, proposal.politicians_id, temasUFC[proposal.tema], proposal.titulo, subtema],
					function(err, rows, fields) {
						if (err) console.log(err);
					});
				}
