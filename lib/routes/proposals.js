		...
		return ddb.scan('proposals', {}, function (err, proposals) {
			if (!err) {
				...
				for(var i=0;i<req.params.qtd;i++){
					...
					var index = parseInt(Math.random() * politicians[probabilityArray[randPolitician]].length);
					items.push(proposals.items[politicians[probabilityArray[randPolitician]][index]]);

					//retira das possibilidades a proposta escolhida
					politicians[probabilityArray[randPolitician]].splice(politicians[probabilityArray[randPolitician]].indexOf(index),1);
					...
					if(politicians[probabilityArray[randPolitician]].length > 0){
