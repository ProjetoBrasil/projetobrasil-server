...
exports.generatesGraphById = function(req,res){
	return ddb.scan('ratings', {filter:{id:{eq:req.params.id}},exclusiveStartKey: {hash: req.params.id, range : ' '}}, function (err, ratings) {
		if (!err) {
...
var calcMedia = function(_id, table, field){
	table = table || 'ratings';
	field = field || 'media';

	return ddb.scan(table, {filter:{id:{eq:_id}},exclusiveStartKey: {hash:_id, range : ' '}}, function (err, ratings) {
		if (!err) {
			var graph = generateGraph(ratings.items, "Brasil");

			return proposals.updateMedia(_id,graph.valor, field);
		} else {
			return console.log(err);
		}
	});
