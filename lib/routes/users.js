...
returnProposals = function(res, user){
	var filter;
	if(user.blindRatedProposals.length == 0)
		return res.send(200,{});
	if(user.blindRatedProposals.length == 1)
		filter = {filter : { id: { eq : user.blindRatedProposals[0]}}}
	else
		filter = {filter : { id: { in : user.blindRatedProposals}}}
	...
	return ddb.getItem('accounts', req._passport.session.user, null, {}, function (err, user) {
		if (!err) {
			if(user.imported == undefined){
				//import dados
				//preenche os dados do user para resolver o problema
				...
				promise.then(function(result) {
					...
					result.forEach(function(element){
						blindRatedProposals.push(element.id);
						blindRating.push(element.nota);
					});

					user.blindRatedProposals = blindRatedProposals;
					user.blindRating = blindRating;
					user.imported = true;
					...
					updatedUser.imported = {value: 0};
					updatedUser.blindRatedProposals = {value: blindRatedProposals};
					updatedUser.blindRating = {value: blindRating};
