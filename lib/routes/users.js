...
returnRatingsProposals = function(res, user){
	...
	user.ratedProposals.forEach(function(rated, index){
		rating.push(parseInt(rated.substr(rated.length - 1)));
		ratedProposals.push(rated.substr(0,rated.length - 1));
	});

	if(ratedProposals.length == 0)
		return res.send(200,[]);
	if(ratedProposals.length == 1)
		filter = {filter : { id: { eq : ratedProposals[0]}}}
	else
		filter = {filter : { id: { in : ratedProposals}}}

	ddb.scan('proposals', filter, function (err, proposals) {
		if (!err) {
			proposals.items.forEach(function(element){
				ratedProposals.forEach(function(rated, index){
					if(element.id == rated)
						element.nota = rating[index];
				});
			});
