...
multipleFindUsers = function(resolve, reject, options, result){
	return ddb.scan('accounts', options, function (err, users) {
		if (!err){
			if(result == undefined)
				result = [];
			
			users.items.forEach(function(element){
				var nome = element.nome || '';
				var username = element.username;
				if(username.indexOf("@") > -1)
					result.push(username + ';' + nome + ';');
			});
			
			if(users.lastEvaluatedKey.hash == undefined){
				return resolve(result);
			}else{
				options.exclusiveStartKey = {hash: users.lastEvaluatedKey.hash, range : users.lastEvaluatedKey.range};
...
exports.findAll = function (req, res){
	...
	var promise = new Promise(function(resolve, reject){
		multipleFindUsers(resolve,reject, {});
	});
