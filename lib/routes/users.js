...
/*exports.findCompleteBlindRatingsById = function (req, res){
	return ddb.scan('blindRatings', {filter : { username: { eq: req._passport.session.user }}}, function (err, ratings) {
		if (!err){
...
multipleFindBlindRatings = function(resolve, reject, options, result, count){
	if(count == undefined) count = 1;
	console.log("Nova Chamada ao banco: "+count.toString());
	if(count ==)
	return ddb.scan('blindRatings', options, function (err, ratings) {
		if (!err){
			...
			console.log(ratings);
			if(ratings.lastEvaluateKey == {}){
				return resolve(result);
			}else{
				options.exclusiveStartKey = ratings.lastEvaluateKey;
				multipleFindBlindRatings(resolve,reject,options, result, count+1);
			}
		}else {
			return reject(err);
		}
...
exports.findCompleteBlindRatingsById = function (req, res){
	...
	promise.then(function(result) {
		...
		return ddb.scan('proposals', {filter : { id: { in: prop }}}, function (err, proposals) {
			if (!err) {
